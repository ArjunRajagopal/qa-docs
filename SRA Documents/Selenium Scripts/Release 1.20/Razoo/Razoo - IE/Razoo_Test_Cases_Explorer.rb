...
# 14-July-2011 - Update - Release 1.20.
#-----------------------------------------------------------------------------#
...
def createRazooAccount(first_name, last_name, email_id, pass_word)
	...
	if @createaccountbutton_topposition == '601'
...
def login(email_id, pass_word)
	...
	if @loginbutton_topposition == '324'
...
def changeMyPassword(new_password)
	...
	if @accountsettingslink_topposition == '185'
	...
	if @savechangesbutton_topposition == '426'
...
def searchOrg(org_name)
	...
	if @donatebutton_topposition == '92' or @donatebutton_topposition == '8'
	...
	if @procheckbox_topposition == '224' or @procheckbox_topposition == '189'
	...
	if @frcheckbox_topposition == '224' or @frcheckbox_topposition == '189'
	...
	if @searchbutton_topposition == '177' or @searchbutton_topposition == '142'
...
def donateUsingNewCard(donation_amount, name_on_card, street, city, state, pin_code, card_number, sec_code, email_id)
	...
	if @donatenowbutton_topposition == '1305' or @donatenowbutton_topposition == '1256' or @donatenowbutton_topposition == '1279'
...
def createOfflineDonation(offline_donorname, offline_donoremail, offline_donoramount)
	...
	if @offlinelink_topposition == '291' or @offlinelink_topposition == '289'
	...
	if @addonationbutton_topposition == '601' or @addonationbutton_topposition == '599'
		...
		if @addonationbutton_topposition == '554' or @addonationbutton_topposition == '575'
			puts "UI Check : Pass. Addonation button is present at the correct position in Offline screen(Team)."
		else
			puts "UI Check : Fail. Addonation button is not present at the correct position in Offline screen(Team)."
		end
	end
	$browser.click "//button[@type='submit']"
	$browser.wait_for_page_to_load 
	...
	if @viewlink_topposition == '313' or @viewlink_topposition == '311'
...
def normalUserSharePage()
	...
	if @imdonelink_topposition == '583' or @imdonelink_topposition == '548'
...
def postComments(donor_comment)
	...
	if @postbutton_topposition == '322' or @postbutton_topposition == '804' or @postbutton_topposition == '310'
...
def createStoryPage(ein_number, org_name, story_name, story_summary, page_story, youtube_url, desig_value, goal_amount,  goal_date, admin_links, sug_amount, sug_desc)
	...
	begin
		...
		$browser.wait_for_page_to_load 
		#***************** Button position check *****************#
		@fundraisebutton_topposition = $browser. "link=Fundraise"
		...
		$browser.click "//a[contains(@href,'?subject=410832480')]"
		$browser.wait_for_page_to_load 
		begin
	...
	begin
		...
		if @createstorybutton_topposition == '1641' or @createstorybutton_topposition == '1617'
	...
	begin
		...
		if @setgoalslink_topposition == '404' or @setgoalslink_topposition == '425' or @setgoalslink_topposition == '380'
	...
	begin
		...
		if @thankdonorlink_topposition == '404' or @thankdonorlink_topposition == '425' or @thankdonorlink_topposition == '380'
	...
	begin
		...
		if @imagevideolink_topposition == '231' or @imagevideolink_topposition == '252' or @imagevideolink_topposition == '207'
		...
		if @savebutton_topposition == '745' or @savebutton_topposition == '766' or @savebutton_topposition == '721'
		...
		@viewlink_topposition = $browser.get_element_position_top "link=View"
		puts "View link Top position : #{@viewlink_topposition}"
		if @viewlink_topposition == '290' or @viewlink_topposition == '311' or @viewlink_topposition == '266'
			puts "UI Check : Pass. View link is present at the correct position in Story screen."
		else
			puts "UI Check : Fail. View link is not present at the correct position in Story screen."
		end
...
def createTeamCampaignSpecificCause(teamcampaign_url, camp_org, camp_title, camp_story, camp_goalamount, camp_goaldate)
	...
	if @createteambutton_topposition == '1676'
...
def createMatchingGrant(mgcreator_name, mg_email, mg_totalamount, mg_date)
	...
	if @mglink_topposition == '252' or @mglink_topposition == '228'
	...
	if @addmgbutton_topposition == '790' or @addmgbutton_topposition == '766'
...
def grantOrgAdminRights(admin_url, email_id, ein_number)
	...
	if @createadminbutton_topposition == '512'
...
def removeOrgAdminRights(org_url, email_id)
	...
	@createadminbutton_topposition = $browser.get_element_position_top "commit"
	puts "CreateAdmin button Top position : #{@createadminbutton_topposition}"
	if @createadminbutton_topposition == '430'
	...
	if @deleteadminlink_topposition == '653'
...
def createOrg(admin_url, ein_number, neworg_name, org_city, org_state)	
	...
	if @createorgbutton_topposition == '501'
...
def editNpoDetails(org_url, first_name, last_name, add_1, add_2, org_emailid, phone_num)	
	...
	@savebutton_topposition = $browser.get_element_position_top "css=button[type=submit]"
	puts "Save button Top position : #{@savebutton_topposition}"
	if @savebutton_topposition == '2915' or @savebutton_topposition == '2831'
...
def auditVerifyOrg(audit_notes, org_url)
	...
	if @verifyauditlink_topposition == '427'
	...
	if @createauditbutton_topposition == '764'
...
def becomeFan()
	...
	if @becomefanlink_topposition == '1050'
...
def impersonatePermission(admin_url, email_id, first_name, last_name)
	...
	if @permissionslink_topposition == '98'
	...
	if @impersonatebutton_topposition == '363'
...
def loginViaGmail(gmail_emailId, gmail_password)
	...
	if @SigninwithGooglebutton_topposition == '6'
...
def logout()
	...
	begin
		assert $browser.is_text_present("Contact Razoo")
			puts "Pass. Contact Razoo label gets displayed at the footer."
		rescue Test::Unit::AssertionFailedError
			puts "Pass. Contact Razoo label is not displayed at the footer."
		end
