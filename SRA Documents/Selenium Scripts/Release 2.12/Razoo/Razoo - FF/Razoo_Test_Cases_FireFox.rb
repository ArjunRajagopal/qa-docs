...
# June 28 2012 - Release 2.8 & 2.9 # July 19 2012 - Release 2.10.  # Aug 13 2012 - Release 2.11
# Sep 4 2012 - Release 2.12
#-----------------------------------------------------------------------------# 
...
def gotoDonFormClickSignup(donation_amount)
	...
	puts "Donate button Top position : #{@donatebutton_topposition}"
	if @donatebutton_topposition == '451'
...
def donateUsingAddCreditCard(donation_amount, street, city, state, pin_code,  card_number, sec_code)
	...
	sleep 5
	begin
		assert $browser.is_element_present("id=recurring_donation_one-time_only")
	...
	$browser.click "id=order_user_order_recurring_donation_weekly"
...
def createStoryPage(ein_number, org_name, story_name, story_summary, page_story, youtube_url, desig_value, goal_amount,  goal_date, admin_links, sug_amount, sug_desc)
	...
	begin
		...
		$browser.wait_for_page_to_load 
		#***************** Button position check *****************#
		@fundraisebutton_topposition = $browser.get_element_position_top "link=Fundraise"
		puts "CreateFR button Top position : #{@fundraisebutton_topposition}"
		if @fundraisebutton_topposition == '54' or @fundraisebutton_topposition == '848'
			puts "UI Check : Pass. Fundraise button is present at the correct position in Story screen."
		else
			puts "UI Check : Fail. Fundraise button is not present at the correct position in Story screen."
		end
...
def purchaseGivingCard (recipient_name, recipient_emailid, recipient_deliverydate, givingcard_amount, givingcard_url)
	...
	begin
		...
		$browser.type "id=order_gift_card_purchase_attributes_title", "Title"
		$browser.type "id=order_gift_card_purchase_attributes_message", "Message"
...
def createTeamCampaignSpecificCause(teamcampaign_url, camp_org, camp_title, camp_story, camp_goalamount, camp_goaldate)
	...
	sleep 3
	$browser.mouse_over("//li[1]/a/strong") #Mouse over on element
	sleep 5
...
def createChallengeGrant(mgcreator_name, mg_email, mg_totalamount, mg_date)
	...
	begin
		...
		puts "@@@@@@@@@ FAILED @@@@@@@@@. Step 1 : CG creation failed."
	end
	begin
...
def closeChallengeGrant(mg_donationamount)
	...
	begin
		...
		puts "@@@@@@@@@ FAILED @@@@@@@@@. CG is not successfully closed."
	end
	sleep 2
	begin
		assert $browser.is_text_present($mgcreator_name)
	...
	begin
		assert $browser.is_text_present("$#{mg_donationamount}.00")
		puts "Pass. Correct donation amount gets displayed successfully in the Grant History section."
	rescue Test::Unit::AssertionFailedError
		puts "Fail. Incorrect donation amount gets displayed successfully in the Grant History section."
	end
	...
	begin
		...
		begin
			...
			begin
				...
				puts "Pass. 'I followed Rajkumar Sithavan' text is displayed under My Activity section."
			rescue Test::Unit::AssertionFailedError
				puts "Fail. 'I followed Rajkumar Sithavan' text is not displayed under My Activity section."
			end
			begin
				assert $browser.is_text_present("Rajkumar Sithavan donated $50 to")
				puts "Pass. 'Rajkumar Sithavan donated $50 to' text is displayed under Follow section."
			rescue Test::Unit::AssertionFailedError
				puts "Fail. 'Rajkumar Sithavan donated $50 to' text is not displayed under Follow section."
			end
			...
			$browser.wait_for_page_to_load
			begin
				assert $browser.is_text_present("People You Follow \n No recent activity found.")
...
# ----------------------------------------------------------------------------------------------------------#
# Description:  This method allows the user to add and remove the wishlist from the story page.
# ----------------------------------------------------------------------------------------------------------#

def wishlist(pro_name)
	
	puts "************** START: Wish List Entry ****************"
	$browser.click "css=img[alt=\"Your profile picture\"]"
	$browser.wait_for_page_to_load 
	$browser.click "link=My Profile"
	$browser.wait_for_page_to_load
	begin
		...
		#$browser.click "link=#{pro_name}"
		$browser.click "xpath=(//a[contains(text(),'donate')])[2]"
		$browser.wait_for_page_to_load 
		begin
			assert $browser.is_text_present("Add to Wish List")
			puts "Passed. Add to Wish List gets displayed in the Story page."
		rescue Test::Unit::AssertionFailedError
			puts "Failed. Add to Wish List not displayed in the Story page."
		end
		puts "************** Clisks on 'Add To Wish List Entry'  ****************"
		
		$browser.click "css=img.wishlist_link"
		begin
			assert $browser.is_text_present("Adding ...")
			puts "Adding...text is displayed Success!!"
		rescue Test::Unit::AssertionFailedError
			puts "Failed. Adding...text is not displayed."
		end
		sleep 10
		#~ $browser.wait_for_page_to_load 
		$browser.click "css=img[alt=\"Your profile picture\"]"
		...
		$browser.wait_for_page_to_load 
		begin
			assert $browser.is_text_present("Wish List")
			puts "Wish List is displayed in user profile screen Success!!"
		rescue Test::Unit::AssertionFailedError
			puts "Failed. Wish List not displayed in user profile screen ."
		end
		#$browser.click "link=#{pro_name}"
		$browser.click "xpath=(//a[contains(text(),'donate')])[2]"
		$browser.wait_for_page_to_load 
		begin
			assert $browser.is_text_present("Drop from Wish List")
			puts "Passed. 'Drop from Wish List' gets displayed in the Story page."
		rescue Test::Unit::AssertionFailedError
			puts "Failed. 'Drop from Wish List' not displayed in the Story page."
		end
		puts "************** Clisks on 'Drop from Wish List Entry'  ****************"
		$browser.click "css=img.wishlist_link"
		
		begin
			assert $browser.is_text_present("Dropping ...")
			puts "Dropping ...text is displayed Success!!"
		rescue Test::Unit::AssertionFailedError
			puts "Failed. Dropping ...text is not displayed."
		end
		sleep 10
		...
		begin
			assert $browser.is_text_present("Wish List")
			puts "Failed. Wish List not removed from user profile screen!!"
		rescue Test::Unit::AssertionFailedError
			puts "Wish List removed from user profile screen Success!!"
		end
	rescue Test::Unit::AssertionFailedError
		puts "@@@@@@@@@ FAILED @@@@@@@@@. #{pro_name} story link is not present in the Profile screen."
	end
	puts "************** END: Wish List Entry ****************"
	
end	


def addCausesByAmount()
	
	puts "************** START: Adding URN to Causes By Amount field ****************"
	$browser.open "/admin/causes_by_amount"
	$browser.wait_for_page_to_load "30000"
	begin
	        puts "Success! user reached Causes by Amount (from Browse Page) screen."
		assert $browser.is_text_present("Causes by Amount (from Browse Page)")
		puts "Passed. 'Causes by Amount (from Browse Page)' text gets displayed in the admin page."
	rescue Test::Unit::AssertionFailedError
		puts "Failed. 'Causes by Amount (from Browse Page)' text not displayed in the admin page."
	end
	
	begin
	        assert $browser.is_text_present("Edit the list of the suggested causes (list one URN per line)")
		puts "Passed. 'Edit the list of the suggested causes (list one URN per line)' text gets displayed in the admin page."
	rescue Test::Unit::AssertionFailedError
		puts "Failed. 'Edit the list of the suggested causes (list one URN per line)' text not displayed in the admin page."
	end
	puts "User Entering Harvard-College URN in the 'Causes by Amount' field"
	$browser.type "id=suggested_urns", "harvard-college"
	$browser.click "css=button[type=\"submit\"]"
	sleep 10
	begin
	        assert $browser.is_text_present("All urns had been parsed and successfully added")
		puts "Passed. All urns had been parsed and successfully added text displayed."
	rescue Test::Unit::AssertionFailedError
		puts "Failed. Adding URN gets failed"
	end
	puts "************** END: Adding URN to Causes By Amount field ****************"
	
	puts "************** START: Verify suggested amount story page in the search screen ****************"
	$browser.click "css=input[type=\"image\"]"
	$browser.wait_for_page_to_load 
	begin
	        assert $browser.is_text_present("How Can I Help for")
		puts "Passed. How Can I Help? for text displayed in the search screen."
	rescue Test::Unit::AssertionFailedError
		puts "Failed. How Can I Help? for text not displayed in the search screen."
	end
	
	begin
	        assert $browser.is_element_present("link=Harvard College")
		puts "Harvard College Story page gets displayed with suggessted donation amount.Perfect."
	rescue Test::Unit::AssertionFailedError
		puts "Failed. Harvard College Story page not displayed"
	end
	puts "************** END: Verify suggested amount story page in the search screen ****************"
	
end
	
def removeCausesByAmount()
	
	puts "************** START: Removing URN from Causes By Amount field ****************"
	...
	puts "Removing Harvard-College URN in the 'Causes by Amount' field"
	$browser.type "id=suggested_urns", ""
	$browser.click "css=button[type=\"submit\"]"
	sleep 10
	puts "************** START: Verify suggested amount story page in the search screen ****************"
	...
	begin
	        assert $browser.is_text_present("How Can I Help for")
		puts "Passed. How Can I Help? for text not displayed in the search screen."
	rescue Test::Unit::AssertionFailedError
		puts "Failed. How Can I Help? for text displayed in the search screen."
	end
	
	begin
	        assert $browser.is_element_present("link=Harvard College")
		puts "Failed: Harvard College Story page displayed with suggessted donation amount"
	rescue Test::Unit::AssertionFailedError
		puts "Passed: Harvard College Story page not displayed with suggessted donation amount"
	end
	puts "************** END: Verify suggested amount story page in the search screen ****************"
