...
def searchOrg(org_name)
	...
	#***************** Button position check *****************#
	@donatebutton_topposition = $browser.get_element_position_top "//strong"
...
def donateUsingNewCard(donation_amount, name_on_card, street, city, state, pin_code, card_number, sec_code, email_id)
	...
	begin
	#GUEST USER DONATION
		assert $browser.is_text_present("(for your receipt)")
...
def donateUsingSavedCard(donation_amount, tip_amount)
	...
	sleep 3	
	begin
		assert $browser.is_text_present("Your card will be charged $#{total_amount}.00")
		puts "Step 3 : Passed. Correct Total amount gets displayed under 'Your card will be charged' section"
	rescue Test::Unit::AssertionFailedError
		puts "Step 3 : Failed. Incorrect Total amount gets displayed under 'Your card will be charged' section"
	end
...
def checkMydonation(donation_amount)
	...
	#***************** Link position check *****************#
	@myaccountlink_topposition = $browser.get_element_position_top "myAccountHeaderLink"
...
def removeFan(story_name)
	...
	$browser.click "//ol[4]/li[1]/div[2]/a/img"
...
def createStoryPage(ein_number, org_name, story_name, story_summary, page_story, youtube_url, desig_value, goal_amount,  goal_date, admin_links, sug_amount, sug_desc)
	...
	begin
		...
		#***************** Link position check *****************#
		@mynpolink_topposition = $browser.get_element_position_top "link=My NPO"
		puts "MyNPO link Top position : #{@mynpolink_topposition}"
		if @mynpolink_topposition == '258'
			puts "UI Check : Pass. MyNPO link is present at the correct position in Home screen."
		else
			puts "UI Check : Fail. MyNPO link is not present at the correct position in Home screen."
		end
		$browser.click "link=My NPO"
		$browser.wait_for_page_to_load 
		#***************** Button position check *****************#
		@createfrbutton_topposition = $browser.get_element_position_top "link=Create a fundraiser"
		...
		begin
			...
			assert $browser.is_element_present("link=Create My Fundraising Page")
			puts "Reached CMS page.:)"
			#$browser.click "//a[contains(@href, '/story/new?subject=#{ein_number}')]"
			$browser.click "link=Create My Fundraising Page"
			$browser.wait_for_page_to_load 
		rescue Test::Unit::AssertionFailedError
		end
	rescue
	...
	#***************** Button position check *****************#
	@createstorybutton_topposition = $browser.get_element_position_top "//button[@type='submit']"
	...
	begin
		assert $browser.is_element_present("link=Images/Videos")
		#***************** Link position check *****************#
		@imagevideolink_topposition = $browser.get_element_position_top "link=Images/Videos"
		puts "ImageVideo link Top position : #{@imagevideolink_topposition}"
		if @imagevideolink_topposition == '258'
			puts "UI Check : Pass. ImageVideo link is present at the correct position in Story screen."
		else
			puts "UI Check : Fail. ImageVideo link is not present at the correct position in Story screen."
		end
		$browser.click "link=Images/Videos"
		$browser.wait_for_page_to_load 
		puts "Step 3 : Upload valid youtube and vimeo video."
		...
		begin
			...
			puts "Step 4 : Passed. Uploaded video gets saved successfully!"
		rescue Test::Unit::AssertionFailedError
			puts "Step 4 : Failed. Uploaded video is not saved successfully!"
		end
		#***************** Link position check *****************#
		@viewlink_topposition = $browser.get_element_position_top "link=View"
		puts "View link Top position : #{@viewlink_topposition}"
		...
		begin
			...
			puts "Step 5 : Passed. Uploaded video gets displayed successfully in Thumbnail section of Story page."
		rescue Test::Unit::AssertionFailedError
			puts "Step 5 : Failed. Uploaded video is not displayed successfully in Thumbnail section of Story page."
		end
	...
	begin
		...
		puts "Step 6 : Passed. Admin links are available in the Created #{story_name} page"
	rescue 
	end
...
def editTeamCampaign(leaderboard_order, camp_url)
	...
	#***************** Link position check *****************#
	@editpagelink_topposition = $browser.get_element_position_top "link=Edit Page"
	puts "Editpage link Top position : #{@editpagelink_topposition}"
	if @editpagelink_topposition == '258'
		puts "UI Check : Pass. Editpage link is present at the correct position in EditTeam screen."
	else
		puts "UI Check : Fail. Editpage link is not present at the correct position in EditTeam screen."
	end
...
def createTeamFundraiserTemplate(story_summary, camp_goalamount)
	...
	$browser.click "//a[@id='create_a_page']/em"
	$browser.wait_for_page_to_load 
	begin
...
def createTeamCampFundraiser(teamcamp_fr, story_summary, story_name, youtube_url, sug_amount, sug_desc, camp_goalamount)
	...
	$browser.type "story_short_description", story_summary
	$browser.type "//body[@id='tinymce']", story_name
	$browser.click "//a[@id='story_long_description_numlist']/span"
	$browser.type "story[donation_suggestion_levels][0]", sug_amount
	$browser.type "story[donation_suggestion_descriptions][0]", sug_desc
...
def createChallengeGrant(mgcreator_name, mg_email, mg_totalamount, mg_date)
	...
	begin
		assert $browser.is_text_present("1 donation left to unlock this $#{$mg_totalamount} challenge grant, provided by Challenge Grant.")
	...
	$browser.wait_for_page_to_load 
	begin
		assert $browser.is_text_present("1 more donation needed to unlock this challenge grant of $101")
...
def createOrg(admin_url, ein_number, neworg_name, org_city, org_state)	
	...
	$browser.wait_for_page_to_load 
	begin
		assert $browser.is_text_present("This NPO is not verified. See its audit history for details.")
...
def editNpoDetails(org_url, first_name, last_name, add_1, add_2, org_emailid, phone_num)	
	...
	$browser.click "document.forms[2].elements[40]"
	$browser.wait_for_page_to_load 
	sleep 5
	begin
