...
# 19-May-2011 - Update - Release 1.16.
#-----------------------------------------------------------------------------#
...
def donateUsingSavedCard(donation_amount, tip_amount)
	...
	sleep 3	
	begin
		assert $browser.is_text_present("Your card will be charged $#{total_amount}")
		puts "Step 3 : Passed. Correct Total amount gets displayed under 'Your card will be charged' section"
	rescue Test::Unit::AssertionFailedError
		puts "Step 3 : Failed. Incorrect Total amount gets displayed under 'Your card will be charged' section"
	end
...
def createStoryPage(ein_number, org_name, story_name, story_summary, page_story, youtube_url, desig_value, goal_amount,  goal_date, admin_links, sug_amount, sug_desc)
	...
	begin
		...
		begin
			assert $browser.is_text_present("My NPO")
			puts "Pass. My NPO link is displayed at the header."
		rescue Test::Unit::AssertionFailedError
			puts "Fail. My NPO link is not displayed at the header."
		end
		#***************** Link position check *****************#
	...
	$browser.click "link=Tell Your Story"
	$browser.wait_for_page_to_load 
	...
	begin
		...
		$browser.click "document.forms[2].elements[23]"
		$browser.wait_for_page_to_load 
	rescue
		...
		$browser.click "document.forms[2].elements[15]"
		$browser.wait_for_page_to_load 
	end
	#***************** Button position check *****************#
...
def verifyNpoDetails()	
	...
	begin
		assert $browser.is_element_present("link=Set Your Goals")
		puts "Passed. Set Your Goals link is displayed under NPO Completion Steps section."
	rescue Test::Unit::AssertionFailedError
		puts "Failed. Set Your Goals link is not displayed under NPO Completion Steps section."
	end
	begin
		assert $browser.is_element_present("link=Share It With Others")
