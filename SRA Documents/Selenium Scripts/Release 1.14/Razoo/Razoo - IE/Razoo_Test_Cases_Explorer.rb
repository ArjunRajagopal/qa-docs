...
def createStoryPage(ein_number, org_name, story_name, story_summary, page_story, youtube_url, desig_value, goal_amount,  goal_date, admin_links, sug_amount, sug_desc)
	...
	begin
		...
		$browser.type "new_video_urls", "http://vimeo.com/15980746\nhttp://www.youtube.com/watch?v=HNVSI_r9SVg"
...
# ------------------------------------------------------------------------------------------------#
# Description:  This method allows the Team Captain to view the Leaderboard.
# ------------------------------------------------------------------------------------------------#
...
def closeMatchingGrant(mg_donationamount)
	...
	begin
		assert $browser.is_text_present("$#{$mg_totalamount} left")
		puts "MG amount is not reached; MG amount is still displayed in the Story page."
	rescue Test::Unit::AssertionFailedError
...
def impersonatePermission(admin_url, email_id, first_name)
	...
	sleep 5
	begin
		assert $browser.is_text_present("Impersonate (you can also use email or the profile short token)")
