...
# 27-Feb-2012 -Scripts updated with scenario by scenario execution method with time. #May 18 2012 - Release 2.7
# June 28 2012 - Release 2.8 & 2.9 # July 19 2012 - Release 2.10. # Aug 13 2012 - Release 2.11
# Sep 4 2012 - Release 2.12. # Oct 10 2012 - Release 2.12 # Nov 19 2012 - Release 2.16
# Feb 13 2012 - Release 2.20
# -------------------------------------------------------------# 
require "test/unit"
require "rubygems"
...
$staticValue = 1; $todayDate = 2; $siteRow = 3; $normalUser1 = 5; $normalUser2 = 6;  $normalUser3 = 7;
$personuser = 7; $causeuser = 8; $stripe = 8
$globalAdmin = 9; $other_amounts = 5; 
$visaCard = 13; $amexCard = 14; $masterCard = 15; $discoverCard = 16; $stripeCard = 17;
$createOrg = 21; $storyNames = 25; $createStory = 29;
$createTeamForSingleCause = 33; $createTeamForMultipleCause = 33; $donateAnywere = 37;
$matchingGrant = 41; $offlineDonations = 45; $myMeetup = 49
$givingCard = 53; $school = 57


# -------------------------------------- START : GETTING TEST DATA'S FROM EXCEL ------------------------
$test_data ="K:\\EBI\\Razoo\\Automation\\Selenium\\Release 2.20\\Razoo\\Razoo - FF\\Razoo_Firefox_TestData.xls"
	
	#--------------To print Starting time of script - Start-----------------#
	$start = Time.now
	puts "Started at: #{$start} seconds"
	#--------------To print Starting time of script - End-----------------#
	...
	$normal3_emailId = $workSheet.Cells($normalUser3, "G").value
	$normal3_password = $workSheet.Cells($normalUser3, "H").value
	
	$stripe_emailId = $workSheet.Cells($stripe, "G").value
	$stripe_password = $workSheet.Cells($stripe, "H").value
	...
	$discovercard_number = $workSheet.Cells($discoverCard, "I").value
	$discover_seccode = $workSheet.Cells($discoverCard, "J").value
	
	$stripecard_number = $workSheet.Cells($stripeCard, "I").value
	$stripecard_seccode = $workSheet.Cells($stripeCard, "J").value
	...
	$personalcause_name = $workSheet.Cells($storyNames, "K").value
	...
	$school_name = $workSheet.Cells($school, "D").value
        $school_city = $workSheet.Cells($school, "E").value
	$school_state = $workSheet.Cells($school, "F").value
	$school_url = $workSheet.Cells($school, "G").value
...
# ------------------------------------------------------------------------------------------------------------------------#
	...
	def scenario5()
		...
		changeMyPassword($normal1_password, $normal1_password)
		logout()
...
# ------------------------------------------------------------------------------------------------------------------------------------------------------#
	...
	def scenario10()
		...
		login($normal1_emailId, $normal1_password)
		changeMyPassword($normal1_password, $normal2_password)
		logout()
...
# ------------------------------------------------------------------------------------------------------------------------------------------------------#
	...
	def scenario11()
		...
		#~ #---------------- Run HACK URL - Start ------------------------------------#
		login($globaladmin_emailId, $globaladmin_password)
		runHackURL("/admin/qa_hack/reindex_all", "/admin/qa_hack/memcached_flush", "/admin/qa_hack/regenerate_statistics")
		logout()
		#~ #---------------- Run HACK URL - End ------------------------------------#
		sleep 5
		searchOrg($new_orgname,$org_url)
		gotoDonFormClickSignup($donation_amount)
		createRazooAccountSignupModal($normal2_firstname, $normal2_lastname, $normal2_emailId, $normal2_password)
...
# -------------------------------------------------------------------------------------------------------------------------------------------------------------#
	...
	def scenario17()
		...
		donateUsingNewCard($mg_donationamount, $normal1_nameoncard, $street, $state, 
					$city, $pin_code, $discovercard_number, $discover_seccode, $normal1don_emailId)
...
# -------------------------------------------------------------------------------------------------------------------------------------------------------------#
# SCENARIO 18 : AS A STORYADMIN, ISBAT CREATE A NEW OFFLINE DONATIONS AND VERIFY IN  THE THERMOMETER.																						
# -------------------------------------------------------------------------------------------------------------------------------------------------------------#
...
# ------------------------------------------------------------------------------------------------------------------------------------------------------------------#
# SCENARIO 19 : AS A STORYADMIN, ISBAT ADD/UPDATE/CLEAR MY MEETUP AND VERIFY PROJECT SETUP AFTER 100% COMPLETION.																				
# ------------------------------------------------------------------------------------------------------------------------------------------------------------------#
...
# -------------------------------------------------------------------------------------------------------------------------#
# SCENARIO 20 : AS A STORYADMIN, ISBAT INSTALL MY WIDGET ON FACEBOOK PAGE.																			
# -------------------------------------------------------------------------------------------------------------------------#
	...
	def scenario20()
		...
		login($normal1_emailId, $normal2_password)
		searchOrg($org_name,$org_url)
		installWidget($fb_emailId, $fb_password)
		logout()
...
# -------------------------------------------------------------------------------------------------------------------------------------------------------------#
# SCENARIO 22 : AS A RAZOO GLOBAL ADMIN, ISBAT REMOVE ORG ADMINS FROM THE ORG PAGE.
#
# -------------------------------------------------------------------------------------------------------------------------------------------------------------#
...
# -------------------------------------------------------------------------------------------------------------------------------------------------------------#
# SCENARIO 23 : AS A NORMAL USER, ISBAT VERIFY PERSON AND ORG UNDER 'PERSON I MAY KNOW' & 'CAUSES I MAY LIKE'
...
# -------------------------------------------------------------------------------------------------#
# SCENARIO 24 : AS A STORY ADMIN, DELETE MY STORY PAGE FROM RAZOO.
#
# -------------------------------------------------------------------------------------------------#
...
# -------------------------------------------------------------------------------------------------------------------------------------------------------------#
# SCENARIO 25 : AS A RAZOO GLOBAL ADMIN, ISBAT CREATE 'CAN IMPERSONATE' PERMISSION.
#
# -------------------------------------------------------------------------------------------------------------------------------------------------------------#
...
# -------------------------------------------------------------------------------------------------------------------------------------------------------------#
# SCENARIO 26 : AS RAZOO ADMIN ISBAT CREATE SCHOOL AND MAKE A DONATION
# -------------------------------------------------------------------------------------------------------------------------------------------------------------#	
	def scenario26()
		
		puts "---------------------- START OF SCENARIO 26 ----------------------"
		#Increase Nces Id Value
		$newschool_id = $workSheet.Cells($school, "C").value
		$newschool_id1 = $newschool_id.to_i()+1
		$workSheet.Cells($school, "C").value = $newschool_id1
		$workBook.Save
		$school_nces_id = $newschool_id1
		puts "Old School ID is : #{$newschool_id}"
		login($globaladmin_emailId, $globaladmin_password)
		createSchool($admincon_url,$school_nces_id, $school_name, $school_city, $school_state)
		puts "Old School ID is : #{$school_url}"
		editSchoolDetails($school_url, $org_firstname, $org_lastname, $org_address1, $org_address2, $org_emailId, $org_phonenum)
		sleep 10
		auditVerifyOrg("Verified this School",$school_url)
		verifyNpoDetails()
		becomeFan()
		donateUsingNewCard($donation_amount, $normal2_nameoncard, $street, $state, 
					 $city, $pin_code, $mastercard_number, $mastercard_seccode, $normal2don_emailId)
...
# -------------------------------------------------------------------------------------------------------------------------------------------------------------#
# SCENARIO 27 : AS A GUEST USER, ISBAT CREATE A RAZOO ACCOUNT BY CLICKING THE 'SIGN UP' LINK AT THE TOP HEADER
#										AND FOLLOW OTHER USER PROFILE.
#
# -------------------------------------------------------------------------------------------------------------------------------------------------------------#
	...
	def scenario27()
		...
		createRazooAccountSignupModal($normal2_firstname, $normal2_lastname, $normal3_emailId, $normal3_password)
		followUser($einlessorg_url)
		logout()
...
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
# SCENARIO 28 : AS A STORY ADMIN, CREATE PERSONAL CAUSE (ONLY IN RAZOO)AND SETUP STRIPE ACCOUNT AND MAKE DONATION.
# 					
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
	...
	def scenario28()
		...
		login($normal1_emailId, $normal2_password)
		createPersonalCausePage($personalcause_name, $story_summary, $page_story, 
					$youtube_url, $desig_value, "link=Donations", $stripe_emailId, $stripe_password)
		donateUsingNewCard($amount_cent, $normal1_nameoncard, $street, $state, 
					$city, $pin_code, $stripecard_number, $stripecard_seccode, $normal1don_emailId)
		postComments("Give and be Blessed!!")
		logout()
...
# -------------------------------------------------------------------------------------------------------------------------------------------------------------#
# SCENARIO 29 : AS AN USER ISBAT LINK MY GMAIL/FACEBOOK ACCOUNT WITH THE RAZOO APPLICATION.
#
# -------------------------------------------------------------------------------------------------------------------------------------------------------------#

	def scenario29()
		
		puts "---------------------- START OF SCENARIO 29 ----------------------"
		loginViaGmail($gmail_emailId, $gmail_password)
		logout()
		loginViaFacebook($fb_emailId, $fb_password)
		logout()
		puts "---------------------- END OF SCENARIO 29 ----------------------"
		puts"Duration: #{Time.now - $start} seconds"
...
#This method helps to stop the Current browser
	...
	def test_01_Razoo()
		...
		scenario18() #As Story admin ISBAT add offline donation and verify it.		
		
		scenario19() #As Story admin ISBAT add/update/delete Meet up.	
		
		#scenario21() #As Story admin ISBAT purchase Giving Card.	
		
		scenario22() #As Global Admin ISBAT remove ORG admin rights.
		
		#scenario23() # Verify 'Person I may Know' & 'Causes I may like' section. REMOVED										
		
		scenario24() # As Story admin Delete PRO/FR page.
		
		scenario25() #As RazAdmin ISBAT impersonate.
		
		scenario26() #As Raz Admin Create School and Donate
		
		scenario27() #As a Guest User, ISBAT create a Razoo Account by clicking the 'Sign up' link modal dialog at the Top nav and Follow the User.
		
		scenario28() #As Story admin ISBAT set up Stripe account and make a daontion.
		
		scenario20() #As Story admin ISBAT install widget on my facebook page.	
		
		scenario29() # As a User ISBAT login via Facebook/Gmail options.
		
		scenario30() #Stop browser
