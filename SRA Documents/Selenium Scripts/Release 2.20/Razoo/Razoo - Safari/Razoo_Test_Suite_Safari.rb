...
require "Razoo_Test_Cases_Safari.rb"
require "win32ole"
...
$test_data ="K:\\EBI\\Razoo\\Automation\\Selenium\\Release 2.20\\Razoo\\Razoo - Safari\\Razoo_Safari_TestData.xls"
...
# ------------------------------------------------------------------------------------------------------------------------------------------------------#
	...
	def scenario11()
		...
		#~ #---------------- Run HACK URL - Start ------------------------------------#
		login($globaladmin_emailId, $globaladmin_password)
		runHackURL("/admin/qa_hack/reindex", "/admin/qa_hack/reindex_all", "/admin/qa_hack/memcached_flush", "/admin/qa_hack/regenerate_statistics")
		logout()
		#~ #---------------- Run HACK URL - End ------------------------------------#
		sleep 180
...
# --------------------------------------------------------------------------------------------------------------------------------------------------#
# SCENARIO 28 : AS A STORY ADMIN, CREATE PERSONAL CAUSE AND SETUP STRIPE ACCOUNT AND MAKE DONATION.
# 					
# --------------------------------------------------------------------------------------------------------------------------------------------------#
	
	def scenario28()
		
		puts "------------------------  START OF SCENARIO 31 ----------------------- "
		login($normal1_emailId, $normal2_password)
...
#This method helps to stop the Current browser
	...
	def test_01_Razoo()
		...
		scenario23() # Verify 'Person I may Know' & 'Causes I may like' section.										
