...
def donateUsingNewCard(donation_amount, name_on_card, street, city, state, pin_code, card_number, sec_code, email_id)
	...
	begin
		...
		puts "Pass. I'm Logged in user! 'Create an account' checkbox is not present."
	end
	begin
...
def donationWidget(donation_amount, widget_emailid, widget_fullname, widget_street, widget_city, widget_state, widget_pcode, card_number, sec_code, tip_amount)
	...
	begin
		assert $browser.is_text_present("I accept the terms of use and privacy policy")
		puts "Pass. 'Terms of use' checkbox is present in the Donation Widget screen."
		$browser.click "id=order_user_order_terms_of_use"
	rescue Test::Unit::AssertionFailedError
		puts "Fail. 'Terms of use' checkbox is not present in the Donation Widget screen."
	end
	$browser.click "//button[@type='submit']"
	sleep 1
	begin
...
def createTeamOfflineDonation(modified_camp_url, normal2_firstname, normal2_lastname, camp_title, offline_donorname, offline_donoremail, offline_donoramount)
	...
	puts "Fail. Offline link is not available for this story page."
	end
...
def checkMydonation(donation_amount)
	...
	begin
		...
		puts "Success!. My donation record gets displayed under 'News Feed' section."
	rescue Test::Unit::AssertionFailedError
		puts "Failure!. My donation record is not displayed under 'News Feed' section."
	end
	begin
		assert $browser.is_text_present("$#{donation_amount}")
		puts "Success!. My donation Amount gets displayed under 'News Feed' screen."
	rescue Test::Unit::AssertionFailedError
		puts "Failure!. My donation Amount doesn't get displayed under 'News Feed' screen."
	end
	#***************** Link position check *****************#
...
def deleteComments(donor_comment)
	...
	#~ @postbutton_topposition = $browser.get_element_position_top "comment_form_submit"
	#~ puts "Post button Top position : #{@postbutton_topposition}"
	#~ if @postbutton_topposition == '951' or @postbutton_topposition == '1090'
...
def removeFan(story_name)
	...
	$browser.click "//ol[4]/li/div[2]/a/img"
	...
	$browser.click "//a[contains(text(),'Harvard College for')]"
	$browser.wait_for_page_to_load 
	
	begin
		assert_equal "Be the first", $browser.get_text("css=button.link.message")
...
def createStoryPage(ein_number, org_name, story_name, story_summary, page_story, youtube_url, desig_value, goal_amount,  goal_date, admin_links, sug_amount, sug_desc)
	...
	begin
		...
		$browser.wait_for_page_to_load 
		#***************** Button position check *****************#
		@fundraisebutton_topposition = $browser.get_element_position_top "//a[contains(@href,'?subject=#{ein_number}')]"
	...
	begin
		assert $browser.is_text_present("Your page is 72% complete.")
		puts "Step 11 : Passed. 72% complete label gets displayed in Fundraiser completion steps progress bar."
	rescue Test::Unit::AssertionFailedError
...
def createThemedFundraiser(normal1_firstname, normal1_lastname, story_summary, page_story, sug_desc, donation_amount)
	...
	begin
		...
		begin
			...
			puts "Step 2 : Failed. FR page is not created successfully from the Themed Fundraiser."
		end
		...
		begin
			assert $browser.is_text_present("Your project is 72% complete.")
			puts "Pass. 72% complete label gets displayed in completion steps progress bar."
		rescue Test::Unit::AssertionFailedError
		...
		puts "Fail. Make a Wish section is not present in the Story page."
	end
...
def editTeamCampaign(leaderboard_order, camp_url)
	...
	sleep 8
	$browser.click "//a[@id='campaign_about_numlist']/span"
	$browser.click "//a[@id='campaign_about_indent']/span"
	$browser.click "//a[@id='campaign_about_outdent']/span"
	$browser.click "//a[@id='campaign_about_numlist']/span"
...
def addMeetup(meetup_title, meetup_date, meetup_location, meetup_updatedlocation)
	...
	begin
		...
		begin
			...
			puts "Fail. Your meetup is not added successfully."
		end
		...
		begin
			...
			puts "Fail. Your meetup is not saved successfully."
		end
		$browser.click "link=show"
		sleep 2
		begin
		...
		begin
			assert $browser.is_text_present("Project Setup")
			puts "Fail. Project Setup section is still get displayed in the story page even after it reaches 100%."
		rescue Test::Unit::AssertionFailedError
			puts "Pass. Project Setup section is not displayed in the story page once it reaches 100%."
		end
	rescue
		puts "Fail. Meetup link is not present in the Story page."
	end
...
def clearMeetup()
	...
	begin
		...
		begin
			...
			puts "Fail. Your meetup is not cleared successfully."
		end
		begin
		...
		puts "Fail. Your Meetup section is not displayed in the Edit story page."
	end
...
def impersonatePermission(admin_url, email_id, first_name, last_name)
	...
	begin
		...
		puts "Step 3 : Failed. Razoo Admin failed impersonating as #{@user_name}."
	end
	rescue
...
def causesAndPersonIlike(org2_url, org3_url, new_orgname3, first_name1, last_name, email_id1, first_name2, email_id2, pass_word)
	...
	$browser.wait_for_page_to_load 
	$browser.type "transactionA", "10"
	$browser.click "css=p.action > button[type=submit]"
	$browser.wait_for_page_to_load 
	$browser.type "order_user_order_credit_card_attributes_address1", "Street"
	$browser.type "order_user_order_credit_card_attributes_city", "city"
	$browser.select "card_state_select", "label=Kansas"
	$browser.type "order_user_order_credit_card_attributes_postal_code", "95008"
	$browser.type "cardNumber", "4111111111111111"
	$browser.type "order_user_order_credit_card_attributes_security_code", "123"
	begin
		assert $browser.is_text_present("I accept the terms of use and privacy policy")
		puts "Pass. 'Terms of use' checkbox is present in the Donation Form screen."
		$browser.click "id=order_user_order_terms_of_use"
	...
	$browser.type "order_user_order_credit_card_attributes_address1", "Street"
	$browser.type "order_user_order_credit_card_attributes_city", "city"
	$browser.select "card_state_select", "label=Alabama"
	$browser.type "order_user_order_credit_card_attributes_postal_code", "95008"
	$browser.type "cardNumber", "4111111111111111"
	$browser.type "order_user_order_credit_card_attributes_security_code", "123"
	begin
	...
	begin
		assert $browser.is_text_present("1 Person I May Know")
		puts "Pass. 1 People I May Know is displayed under User2 Profile screen."
	rescue Test::Unit::AssertionFailedError
		puts "Fail. 1 People I May Know is not displayed under User2 Profile screen."
	end
	begin
		assert $browser.is_text_present("1 Cause I May Like")
		puts "Fail. 1 Cause I May Know is displayed under User2 Profile screen."
	rescue Test::Unit::AssertionFailedError
		puts "Pass. 0 Cause I May Know is displayed under User2 Profile screen."
	end
	$browser.click "link=Log out"
	$browser.wait_for_page_to_load
	...
	begin
		assert $browser.is_text_present("1 Person I May Know")
		puts "Pass. 1 People I May Know is displayed under User1 Profile screen."
	rescue Test::Unit::AssertionFailedError
		puts "Fail. 1 People I May Know is not displayed under User1 Profile screen."
	end
	begin
		assert $browser.is_text_present("1 Cause I May Like")
		puts "Pass. 1 Cause I May Know is displayed under User1 Profile screen."
	rescue Test::Unit::AssertionFailedError
		puts "Fail. 1 Cause I May Know is not displayed under User1 Profile screen."
	end
