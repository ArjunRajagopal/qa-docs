...
def searchProject(project_name)
	...
	begin
		assert $browser.is_text_present("continue without commenting")
		$browser.click "link=continue without commenting"
		$browser.wait_for_page_to_load
	rescue Test::Unit::AssertionFailedError
	end
...
def postComments(donor_comment, share_emailid, story_url)
	...
	normalUserSharePage(share_emailid, story_url)
...
def deleteComments(donor_comment, story_url)
	
	puts "************** START : Delete Comment ****************"	
	$browser.click "link=continue without commenting"
	$browser.wait_for_page_to_load 
	$browser.click "link=Skip this step"
	$browser.wait_for_page_to_load 
	$browser.open story_url
	$browser.wait_for_page_to_load 
...
def editTeamCampaign(leaderboard_order, camp_url)
	...
	$browser.type "//body[@id='tinymce']", "Story"
	########### ONLY FOR IE #############
	$browser.click "//a[@id='campaign_about_numlist']/span"
