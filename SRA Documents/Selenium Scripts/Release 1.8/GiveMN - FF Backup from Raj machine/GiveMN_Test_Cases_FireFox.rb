...
#------------------------------------------------------------------#
# Description:  It is a METHOD for invoking the GiveMN Site
#------------------------------------------------------------------#

def invokeRazooSite()
	
	puts "************** START : Invoke GiveMN Application ****************"	
	# Create a new instance of the Selenium-Client driver.
	$browser = Selenium::Client::Driver.new \
		:host => "localhost",
		:port => 4444,
		:browser => "*firefox", 
		:url => "https://givemn-stage.razoo.com/login",
	...
	$browser.set_context("GiveMN Application")
	$browser.open "/"
	puts "Passed. GiveMN Site gets invoked successfully!"
	puts "************** END : Invoke GiveMN Application ****************"
	
end

#-----------------------------------------------------------------------------#
# Description:  This method allows the user to create account in GiveMN.
#-----------------------------------------------------------------------------#

def createRazooAccount(first_name, last_name, email_id, pass_word)
	
	puts "************** START : Create a GiveMN Account ****************"	
	begin
	...
	begin
		assert $browser.is_text_present("Create an account on GiveMN")
	...
	puts "************** END : Create a GiveMN Account ****************"	
end

# ------------------------------------------------------------------------------#
# Description:  This method allows the user to login to GiveMN application.
# ------------------------------------------------------------------------------#
def login(email_id, pass_word)
	
	puts "************** START : Login to GiveMN ****************"	
...
def searchOrg(org_name)
	...
	$browser.wait_for_page_to_load
	begin
		assert_equal "#{$story_name} - GiveMN", $browser.get_title
...
def gotoDonFormClickSignup(donation_amount)
	...
	$browser.wait_for_page_to_load 
	begin
		assert_equal "An inspiring place to donate and raise money - GiveMN", $browser.get_title
...
def donateUsingNewCard(donation_amount, name_on_card, street, city, state, pin_code, card_number, sec_code, email_id)
	...
	begin
		...
		puts "Step 3 : Passed. Donation was Successful!"
	rescue Test::Unit::AssertionFailedError
...
def donateUsingAddCreditCard(donation_amount, street, city, state, pin_code,  card_number, sec_code, donate_weekly)
	...
	begin
		assert $browser.is_text_present("Your donation was successful! We've sent you a confirmation email receipt.")
		puts "Step 5 : Passed. Donation was Successful!"
	rescue Test::Unit::AssertionFailedError
...
def guestUserSharePage(share_emailid)
	...
	begin
		assert_equal "#{$story_name} - GiveMN", $browser.get_title
		puts "Step 2 : Passed. User has reached the #{$story_name} story page successfully."
	rescue Test::Unit::AssertionFailedError
		puts "Step 2 : Failed in reaching the #{$story_name} story page."
	end
		
	begin
		assert $browser.is_text_present("Thank you for telling others about this cause!")
		puts "Step 3 : Passed. User has successfully shared the #{$story_name} story page."
	rescue Test::Unit::AssertionFailedError
		puts "Step 3 : Failed in sharing the #{$story_name} story page."
	end
...
def normalUserSharePage(share_emailid)
	...
	begin
		assert $browser.is_text_present("Thank you for telling others about this cause! Leave a comment below to show your support!")
		puts "Step 3 : Passed. User has successfully shared the #{$story_name} story page."
	rescue Test::Unit::AssertionFailedError
		puts "Step 3 : Failed in sharing the #{$story_name} story page."
	end
...
def editNpoDetails(first_name, last_name, add_1, add_2, org_emailid, phone_num)	
	...
	$browser.select "org_state", "label=Minnesota"
...
def verifyNpoDetails()	
	...
	$browser.wait_for_page_to_load 
	begin
		assert_equal "#{$neworg_name} - GiveMN", $browser.get_title
...
def logout()
	...
	puts "************** START : Log out from GiveMN ****************"
	sleep 2
