...
$test_data ="D:\\EBI\\MyProjects\\Razoo\\Automation\\Selenium\\Selenium2\\ModularFramework\\Arjun\\Release 2.5\\Razoo\\Razoo - Safari\\Razoo_Safari_TestData.xls"
	...
	$school_nces_id = $workSheet.Cells($school, "C").value
...
# -------------------------------------------------------------------------------------------------------------------------------------------------------------#
	...
	def scenario30()
		...
		login($globaladmin_emailId, $globaladmin_password)
		$school_nces_id = $workSheet.Cells($school, "C").value
		$school_nces_id_afterincr = $school_nces_id.to_i()+1
		$workSheet.Cells($school, "C").value = $school_nces_id_afterincr
                $workBook.Save
		...
		#~ editNpoDetails($org_url, $org_firstname, $org_lastname, $org_address1, $org_address2, $org_emailId, $org_phonenum)
		sleep 10
		auditVerifyOrg("Verified this School", $org_url)
		verifyNpoDetails()
		becomeFan()
		donateUsingSavedCard($donation_amount,  $tip_amount)
		postComments("Give and be Blessed!!")
		checkMydonation($donation_amount, $school_name)
		logout()
		puts "---------------------- END OF SCENARIO 1 --------------------------"
		puts"Duration: #{Time.now - $start} seconds"
	end
